"""
Section 5: Customer Rights Generation - Open-Source Approach
...
# Configuration
TEMPLATE_PATH = os.path.join(os.path.dirname(__file__), "..", "assets", "templates", "pc-templates", "S5_Rights_as_Customer_Placeholder.docx")
OUTPUT_DIR = os.path.join(os.path.dirname(__file__), "..", "output")
LIBREOFFICE_PATH = r"C:\Program Files\LibreOffice\program\soffice.exe"


def generate_s5_customer_rights(rights_data: dict = None):
    """
    Generate Section 5: Your Rights as a Customer document

    Args:
        rights_data: Dictionary containing rights data (mostly static, optional)

    Returns:
        tuple: (docx_path, pdf_path) or (docx_path, None) if PDF fails
    """

    print(f"\n[S5] Generating Customer Rights")
    ...
    docx_filename = f"s5_customer_rights_{timestamp}.docx"
    ...
    # Convert to PDF
    pdf_filename = f"s5_customer_rights_{timestamp}.pdf"
...
if __name__ == "__main__":
    print("=" * 80)
    print("SECTION 5: CUSTOMER RIGHTS GENERATION - OPEN-SOURCE APPROACH")
    print("Library: docxtpl (Jinja2) + LibreOffice")
    print("=" * 80)

    try:
        print(f"\n[1/3] Loading template and generating DOCX...")
        print(f"      Content: Static Customer Rights Disclosure")

        total_start = time.time()
        # No dynamic data needed for this static section
        docx_path, pdf_path, docx_time, pdf_time = generate_s5_customer_rights()
        total_time = time.time() - total_start

        print("\n" + "=" * 80)
        print("GENERATION COMPLETE - SECTION 5")
        print("=" * 80)
        print(f"\nPerformance Metrics:")
