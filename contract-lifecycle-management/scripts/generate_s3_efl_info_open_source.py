"""
Section 3: EFL Info Generation - Open-Source Approach
...
# Configuration
TEMPLATE_PATH = os.path.join(os.path.dirname(__file__), "..", "assets", "templates", "pc-templates", "S3_EFL_Info.docx")
OUTPUT_DIR = os.path.join(os.path.dirname(__file__), "..", "output")
LIBREOFFICE_PATH = r"C:\Program Files\LibreOffice\program\soffice.exe"


def generate_s3_efl_info(efl_data: dict):
    """
    Generate Section 3: EFL Info document

    Args:
        efl_data: Dictionary containing Electricity Facts Label information

    Returns:
        tuple: (docx_path, pdf_path) or (docx_path, None) if PDF fails
    """

    print(f"\n[S3] Generating EFL Info with price: {efl_data.get('contractPrice')}")
    ...
    docx_filename = f"s3_efl_info_{timestamp}.docx"
    ...
    # Convert to PDF
    pdf_filename = f"s3_efl_info_{timestamp}.pdf"
...
SAMPLE_DATA = {
    "contractPrice": "1.57",
    "averagePrice": "1.57",
    "basePricePerKwh": "0.015"
}


if __name__ == "__main__":
    print("=" * 80)
    print("SECTION 3: EFL INFO GENERATION - OPEN-SOURCE APPROACH")
    print("Library: docxtpl (Jinja2) + LibreOffice")
    print("=" * 80)

    try:
        print(f"\n[1/3] Loading template and generating DOCX...")
        print(f"      Contract Price: {SAMPLE_DATA['contractPrice']} ¢/kWh")

        total_start = time.time()
        docx_path, pdf_path, docx_time, pdf_time = generate_s3_efl_info(SAMPLE_DATA)
        total_time = time.time() - total_start

        print("\n" + "=" * 80)
        print("GENERATION COMPLETE - SECTION 3")
        print("=" * 80)
        print(f"\nPerformance Metrics:")
